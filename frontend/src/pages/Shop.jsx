...
const Shop = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 md:py-12 flex flex-col md:flex-row gap-6 md:gap-8">
      {/* Mobile Filter Toggle */}
      <div className="md:hidden">
        <button
          onClick={() => setShowMobileFilters(!showMobileFilters)}
          className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold bg-white shadow-sm border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors"
        >
          <Filter size={20} />
      ...
      {/* Sidebar Filters */}
      <aside className={`w-full md:w-64 shrink-0 transition-all ${showMobileFilters ? "block" : "hidden md:block"}`}>
            ...
            {products.map((product) => (
              <div key={product.id} className="card group flex flex-col">
                <Link
                  ...
                  className="block relative h-48 bg-slate-100 flex items-center justify-center p-4 overflow-hidden"
                >
                  {product.imageUrl ? (
                    <img
