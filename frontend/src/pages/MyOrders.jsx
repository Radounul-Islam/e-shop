        ...
        <div className="space-y-6">
          {orders.map((order) => (
            <div key={order.id} className="card p-6 overflow-hidden relative">
              <div
                className={`absolute top-0 left-0 w-1.5 h-full ${order.status === "DELIVERED" ? "bg-green-500" : "bg-primary-500"}`}
              ></div>
              <div className="flex flex-col sm:flex-row justify-between sm:items-start border-b border-slate-100 pb-4 mb-4 gap-4 sm:gap-0">
                <div>
                  <div className="flex items-center gap-3 mb-1 flex-wrap">
                    ...
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${order.status === "DELIVERED" ? "bg-green-100 text-green-700" : "bg-primary-100 text-primary-700"}`}
                    >
                      {order.status}
                    </span>
                  </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <ExternalLink
                            size={12}
                            className="text-slate-300 group-hover:text-primary-400 opacity-0 group-hover:opacity-100 transition-all"
                          />
                        </div>
                    ...
                    <div className="text-right">
                      ...
                      {order.status === "DELIVERED" && (
                        <div className="text-[10px] text-primary-500 font-bold mt-0.5">
                          Rate & Review
                        </div>
                      )}
                    </div>
