    ...
    <div className="max-w-4xl mx-auto px-6 py-12">
      ...
      {items.length === 0 ? (
        <div className="bg-white p-12 text-center card">
          <h2 className="text-xl font-bold text-slate-600 mb-4">Your cart is empty</h2>
        ...
        <div className="flex flex-col md:flex-row gap-8">
              ...
              <div key={item.product.id} className="card p-4 flex items-center gap-4">
                <div className="h-20 w-20 bg-slate-100 rounded-md flex items-center justify-center shrink-0">
                  {item.product.imageUrl ? (
                    <img src={item.product.imageUrl} alt={item.product.name} className="h-full w-full object-cover rounded-md" />
                  ) : <span className="text-xs text-slate-400">Image</span>}
                </div>
                
                <div className="flex-grow">
                  <h3 className="font-bold text-slate-800">{item.product.name}</h3>
                  <div className="text-primary-600 font-extrabold">${item.product.price.toFixed(2)}</div>
                </div>
          ...
          <div className="w-full md:w-80 shrink-0">
            <div className="card p-6 sticky top-24">
              <h2 className="text-lg font-bold border-b pb-4 mb-4 text-slate-800">Order Summary</h2>
              ...
              <div className="flex justify-between items-center border-t py-4 mb-6">
                ...
                <span className="font-extrabold text-2xl text-primary-600">${getTotal().toFixed(2)}</span>
              </div>
