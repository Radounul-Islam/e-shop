...
import { ArrowLeft, CheckCircle, AlertCircle, ShoppingCart, Star } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 bg-slate-50 min-h-[400px] flex items-center justify-center p-8">
          {product.imageUrl ? (
            <img src={product.imageUrl} alt={product.name} className="w-full h-auto object-cover rounded-xl shadow-lg mix-blend-multiply" />
          ) : (
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-800 mb-4">{product.name}</h1>
      ...
      <div className="mt-16 bg-white rounded-3xl shadow-sm border border-slate-100 p-8 md:p-12">
         <div className="flex flex-col md:flex-row gap-12">
            ...
            <div className="w-full md:w-1/3">
               <h2 className="text-3xl font-extrabold text-slate-800 mb-6">Customer Reviews</h2>
               
               <div className="flex items-center gap-4 mb-8 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                  <div className="text-5xl font-black text-slate-800">{avgRating}</div>
                  <div>
                    <div className="flex text-yellow-400 mb-1">
                       {[1,2,3,4,5].map(star => (
                         <Star key={star} size={20} fill={star <= Math.round(avgRating) ? "currentColor" : "none"} className={star <= Math.round(avgRating) ? "" : "text-slate-300"} />
                       ))}
                    </div>
                    <div className="text-sm font-semibold text-slate-500">Based on {product.reviews?.length || 0} reviews</div>
                  </div>
               </div>

               {user ? (
                  <form onSubmit={handleReviewSubmit} className="bg-white border text-left border-slate-200 p-6 rounded-2xl shadow-sm">
                     <h3 className="font-bold text-lg mb-4 text-slate-800">Write a Review</h3>
                     
                     <div className="mb-4">
                        <label className="block text-sm font-semibold mb-2 text-slate-600">Overall Rating</label>
                        <div className="flex gap-1" onMouseLeave={() => setHoverRating(0)}>
                           {[1, 2, 3, 4, 5].map(star => (
                             ...
                             >
                               <Star size={28} fill={star <= (hoverRating || rating) ? "#facc15" : "none"} className={star <= (hoverRating || rating) ? "text-yellow-400" : "text-slate-300"} />
                             </button>
                           ))}
                        </div>
                     </div>
                     ...
                     <div className="mb-6">
                        <label className="block text-sm font-semibold mb-2 text-slate-600">Your Review (Optional)</label>
                        <textarea 
                     ...
                     <button type="submit" disabled={submittingReview} className="w-full btn-primary py-3 rounded-xl disabled:opacity-50">
                       {submittingReview ? 'Submitting...' : 'Submit Review'}
                     </button>
                  </form>
               ) : (
                  <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl text-center">
                     <h3 className="font-bold text-lg mb-2 text-slate-800">Review this product</h3>
                     <p className="text-slate-500 text-sm mb-4">You must be logged in to share your thoughts.</p>
                     <Link to="/login" className="btn-primary py-2 px-6 inline-block rounded-xl">Login to Review</Link>
                  </div>
               )}
            </div>
            ...
            <div className="w-full md:w-2/3">
               <div className="space-y-6">
                  {(!product.reviews || product.reviews.length === 0) ? (
                     <div className="text-center py-12 text-slate-400 font-medium">No reviews yet. Be the first!</div>
                  ) : (
                     product.reviews.map(rev => (
                        <div key={rev.id} className="border-b border-slate-100 pb-6 mb-6 last:border-0 last:pb-0 last:mb-0">
                           <div className="flex items-center justify-between mb-2">
                              <div className="font-bold text-slate-800">{rev.user?.email || rev.user?.phone || 'Anonymous'}</div>
                              <div className="text-xs font-semibold text-slate-400">{new Date(rev.createdAt).toLocaleDateString()}</div>
                           </div>
                           <div className="flex text-yellow-400 mb-3">
                              {[1,2,3,4,5].map(star => (
                                <Star key={star} size={14} fill={star <= rev.rating ? "currentColor" : "none"} className={star <= rev.rating ? "" : "text-slate-300"} />
                              ))}
                           </div>
